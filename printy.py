...
def printbatch(labeler, labels, printer, picklist):
    ...
    win32api.ShellExecute(0, 'open', GSPRINT_PATH,
                          '-ghostscript "' + GHOSTSCRIPT_PATH + '" -printer "' + labeler + '" "' + labels + '"', '.', 0)
    time.sleep(1)
    print("Done!")
...
if res == "":
    ...
    while x < len(_picklists):
        ...
        if res == "Y":
            ...
            for pick in picklistsToMove:
                shutil.move(pick, _picklistOutgoingFolder + "/" + os.path.basename(pick))
            for lab in labelsToMove:
                shutil.move(lab, _labelOutgoingFolder + "/" + os.path.basename(lab))
            print("Done!")
        else:
